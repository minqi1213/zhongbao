<html>
<head>
  <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
  <title>SunnyTest</title>
  <style type="text/css">
    h1 {color:white; font-size:24pt; text-align:center;
        font-family:arial,sans-serif}
    .menu {color:white; font-size:12pt; text-align:center;
           font-family:arial,sans-serif; font-weight:bold}
    .label_header {background:black}
    p {color:black; font-size:12pt; text-align:justify;
       font-family:arial,sans-serif}
    p.foot {color:white; font-size:9pt; text-align:center;
            font-family:arial,sans-serif; font-weight:bold}
    a:link,a:visited,a:active {color:white}
  </style>
</head>
<body>

  <!-- page header -->
  <table width="100%" cellpadding="12" cellspacing="0" border="0">
  <tr bgcolor="black">
    <td align="left"><img src="logo.gif" alt="TLA logo" height="70" width="70"></td>
    <td>
        <h1>SunnyTest 众包</h1>
    </td>
    <td align="right"><img src="logo.gif" alt="TLA logo" height="70" width="70" /></td>
  </tr>
  </table>

  <!-- menu -->
  <table width="100%" bgcolor="white" cellpadding="4" cellspacing="4">
  <tr >
    <td class="label_header" idth="25%">
      <img src="s-logo.gif" alt="" height="20" width="20" />
      <span class="menu"><a href="login.html">提交</a></span></td>
    <td class="label_header" width="25%">
      <img src="s-logo.gif" alt="" height="20" width="20" />
      <span class="menu"><a href="login.html">查询</a></span></td>
    <td class="label_header" width="25%" >
      <img src="s-logo.gif" alt="" height="20" width="20" />
      <span class="menu" ><a href="reg.html">注册</a></span></td>
    <td class="label_header" width="25%">
      <img src="s-logo.gif" alt="" height="20" width="20" />
      <span class="menu"><a href="login.html">登陆</a></span></td>
  </tr>
  </table>

